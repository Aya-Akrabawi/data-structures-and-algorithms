...
const createServer = () => {
    ...
    app.get('/', function(req, res) {
      res.status(200).send('this is a send');
    })
  
    app.delete('/things/1', function(req, res) {
      res.status(405).send('this is a delete');
    })
  
    app.get('*', function(req, res) {
      res.status(404).send('this is a delete');
    })
...
let biggerThanLuke = (arr) => {
    ...
    let result  = [];
    arr.forEach(el =>{
      if( Number(el.mass) > Number(arr[0].mass) ){
        result .push(el.name)
      }
    })
