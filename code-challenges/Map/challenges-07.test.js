...
const forLoopTwoToThe = (arr) => {
  ...
  for (let index = 0; index < arr.length; index++) {
     ...
     let powered = Math.pow(2, arr[index]);
     newArr.push(powered);
...
Write a function named forEachTwoToThe that produces the same output as your forLoopTwoToThe function from
...
const forEachTwoToThe = (arr) => {
  ...
  let forEachArr = [];
  arr.forEach(element => {
    let  forEachVar = Math.pow(2,element);
    forEachArr.push(forEachVar);
  });
  return forEachArr;
};
