...
const createServer = () => {
  ...
  app.all('*', (req, res) => {
    res.status(404).send({ 'status': 404, responseText: 'Not Found'});
  });
...
const reversedString = (str) => {
  let arr = str.split('');
  arr = arr.reduce((past, element) => element+past, '');
  return arr;
};
...
const countNumberOfChildren = (arr) => arr.reduce((accumulator, characters, index) => {
  (characters.children === undefined ? 0 : accumulator = accumulator + parseInt(characters.children.length));
  return accumulator;
  }, 0);
...
const calculateAverage = (arr) => {
  arr = arr.reduce((accumulator, number) => {
    accumulator.count = accumulator.count + 1;
    accumulator.sum = accumulator.sum + number;
    return accumulator;
    }, { count: 0, sum: 0 });
...
const countPrimeNumbers = (arr) => arr.reduce((accumulator, number) => {
  (isPrime(number) ? accumulator = accumulator + 1 : 0);
  return accumulator;
  }, 0);
...
const extractStat = (statName, arr) => arr.reduce((accumulator, stats) => {
  (stats.stat.name == statName ? accumulator = stats : 0);
  return accumulator;
  }, {});
...
const extractChildren = (arr) => {
  const aExists = (character) => character.includes('a') ;
  const result = arr.filter(character => aExists(character.name.toLowerCase()));
  var finalResult = result.reduce((accumulator, characters, index) => {
    (characters.children === undefined ? 0 : accumulator[index] = characters.children);
    return accumulator;
    }, []);
  finalResult = finalResult.reduce((acc, val) => acc.concat(val), []);
