...
const count = (target, input) => {
  // Solution code here...
  return input.map(element => element.filter(element => element == target ? 1 : 0)).reduce((acc, val) => acc.concat(val), []).length;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, val) => acc.concat(val), []).reduce((acc, val) => acc + val, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(element => element.filter(element => !(typeof element === 'string') && element%5 == 0 ? 1 : 0).map(element => Math.pow(2, element)));
};
